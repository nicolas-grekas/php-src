...
$a = ['value' => 1, 'tag' => 't'];
$a['alias'] = &$a['value'];
var_dump(VC::set('k_scalar', $a) === true);
...
var_dump(VC::get('k_scalar')['value'] === 1);

/* 2. A single (unshared) reference still round-trips through the fast path. */
$x = 10;
$b = ['r' => &$x, 'tag' => 'u'];
var_dump(VC::set('k_single', $b) === true);
echo "single value: ";
var_dump(VC::get('k_single')['r'] === 10);
...
$c['p'] = &$o;
$c['q'] = &$o;
var_dump(VC::set('k_objref', $c) === true);
...
$pair->a = &$v;
$pair->b = &$v;
var_dump(VC::set('k_proptref', $pair) === true);
echo "proptref round-trips to object: ";
var_dump(VC::get('k_proptref') instanceof K_Pair);

$gp = VC::get('k_proptref');
$gp->a = 55;
