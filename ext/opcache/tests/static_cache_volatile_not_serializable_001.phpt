...
echo "fiber set: ";
var_dump(VC::set('k_fiber', $fiber));
echo "fiber stored: ";
var_dump(VC::get('k_fiber'));

echo "generator set: ";
var_dump(VC::set('k_gen', $generator));
echo "generator stored: ";
var_dump(VC::get('k_gen'));

/* Nested inside an array: the whole value is refused. */
echo "nested set: ";
var_dump(VC::set('k_nested', ['x' => $fiber, 'y' => 1]));
echo "nested stored: ";
var_dump(VC::get('k_nested'));

/* A storable value alongside still works. */
echo "control set: ";
var_dump(VC::set('k_ok', ['a' => 1, 'b' => [2, 3]]));
echo "control value: ", VC::get('k_ok')['b'][1], "\n";
?>
--EXPECT--
