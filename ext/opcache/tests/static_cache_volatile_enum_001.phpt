...
VC::set('ek_backed', K_Suit::Hearts);
$g = VC::get('ek_backed');
...
/* A pure enum case. */
VC::set('ek_plain', K_Plain::B);
echo "plain is singleton: ";
var_dump(VC::get('ek_plain') === K_Plain::B);

/* Enum cases nested in an array. */
VC::set('ek_arr', ['suit' => K_Suit::Spades, 'plain' => K_Plain::A, 'n' => 5]);
$ga = VC::get('ek_arr');
echo "array enums: ";
var_dump($ga['suit'] === K_Suit::Spades && $ga['plain'] === K_Plain::A && $ga['n'] === 5);

/* Enum case as an object property. */
VC::set('ek_card', new K_Card(K_Suit::Spades));
echo "card enum: ";
var_dump(VC::get('ek_card')->suit === K_Suit::Spades);
