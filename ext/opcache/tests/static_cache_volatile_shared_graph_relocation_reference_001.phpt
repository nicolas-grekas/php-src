...
var_dump(VC::set('reloc_first', str_repeat('A', 1500000)));
var_dump(VC::set('reloc_graph', build_graph()));
var_dump(VC::set('reloc_third', str_repeat('C', 1500000)));

VC::delete('reloc_first');

/* Forces compaction; the unreferenced graph block is moved and must be rebased. */
var_dump(VC::set('reloc_merged', str_repeat('M', 3200000)));

$g = VC::get('reloc_graph');
echo $g['a']['rs1'], "\n";
echo $g['a']['rs2'], "\n";
var_dump($g['a'] === $g['b']);
