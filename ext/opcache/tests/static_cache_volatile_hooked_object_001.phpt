...
/* A nested __serialize object keeps the whole graph on the fast path (not the
 * opaque serializer), and __serialize() runs exactly once on store. */
$box = new Box();
$box->h = new Hooked(42);
Hooked::$ser = 0;
VC::set('k_box', $box);
